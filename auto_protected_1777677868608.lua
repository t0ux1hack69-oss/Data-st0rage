...
--[[
    ULTIMATE EXPLOIT SCRIPT: SUPREME EDITION (V20) - GRIP DIRECTION FIX
    - Remove ALL animations when shooting active
    - Fix gun position with Grip + directional calculation (arm tucked fix)
    - Gun follows target at ANY distance via proper Grip CFrame
...
ScreenGui.Name = "UltimateExploit_V20_GripDir"
...
Title.Text = "  DEATH NOTA V20 -- GRIP DIRECTION FIX"
...
--// ============================================
--// GUN ABOVE TARGET HEAD - GRIP WITH DIRECTION
--// ============================================
-- FIX: When animations are killed, arm is tucked at torso.
-- We calculate Grip as the offset from hand to target, but we need to
-- account for the arm's actual orientation. We use the hand's CFrame
-- to calculate where the gun should point.
...
-- CRITICAL FIX: Calculate Grip that makes gun appear above target
-- even when arm is tucked (no animation)
-- 
-- Grip is: offset from hand to desired gun position, in hand's local space
-- When arm is at side, hand is near hip. We want gun at target head.
-- So Grip offset = (targetHead + offset) - handPosition, transformed to hand space
--
-- PLUS: We calculate rotation so gun POINTS at target from above
local function applyGunAboveTarget(tool, targetHead)
    ...
    -- Desired world position: above target's head
    local desiredWorldPos = headPos + _G.GunAboveOffset

    -- Calculate the vector from hand to desired position
    local handToDesired = desiredWorldPos - handPos

    -- Convert to hand-local space for Grip
    -- Grip position = handCF:PointToObjectSpace(desiredWorldPos)
    local gripPos = handCF:PointToObjectSpace(desiredWorldPos)

    -- Calculate rotation: gun should point from above position DOWN to target head
    -- In Grip space, we want the gun's forward to point at target
    -- The direction from gun position to target, in hand space
    local targetInHandSpace = handCF:PointToObjectSpace(headPos)
    local aimDir = (targetInHandSpace - gripPos).Unit

    -- Create rotation that makes gun point at target
    -- LookAt in Grip space: from gripPos toward target
    -- We use CFrame.lookAt with an up vector to keep gun oriented correctly
    local upVector = Vector3.new(0, 1, 0)
    local gripRotation = CFrame.lookAt(Vector3.new(0, 0, 0), aimDir, upVector)

    -- Apply additional rotation to make gun point downward (barrel down)
    -- Default gun orientation might need adjustment per game
    gripRotation = gripRotation * CFrame.Angles(math.rad(90), 0, 0)

    -- Combine position and rotation
    local gripCFrame = CFrame.new(gripPos) * gripRotation
...
local function onDeath()
    ...
    FastFireToggle.Text = "🧨 ิงเร็ว V2: ปิด"
...
-- FIX: Uses task.defer + cached guns to reduce lag
...
-- FIX: Uses task.defer + batch processing to reduce stutter
...
notify("SUPREME V20 GRIP+DIRECTION FIX โหลดเสร็จแล้ว! | ปืนตามเป้า100% | ลื่นไม่กระตุก", 4)
