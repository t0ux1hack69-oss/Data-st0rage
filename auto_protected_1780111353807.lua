...
--[[
    PS CITY | MULTI-SYNC EXPLOIT HUB (FIXED CLICK BUG)
    DEVELOPED BY: TOU
    VERSION: 3.1.0
...
-- // Interactive Logic Connections (Using .Activated for 100% click guarantee on all Exploits) // 😈
...
ToggleBtn.Activated:Connect(toggleRainbow) -- Fallback for newer core layouts
...
local function toggleGlitchName()
...
GlitchNameBtn.MouseButton1Click:Connect(toggleGlitchName)
GlitchNameBtn.Activated:Connect(toggleGlitchName) -- Fallback
    ...
    while true do
        if GlitchNameEnabled and SettingsRemote then
            for _, NameString in ipairs(DisplayNameVariants) do
                ...
                task.wait(0.2) -- Optimization delay threshold
            end
        else
            task.wait(0.5)
        end
